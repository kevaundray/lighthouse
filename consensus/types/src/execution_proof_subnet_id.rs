...
/// Number of execution proof subnets (each subnet can represent a different zkVM system)
...
/// Subnets 0-7 are available, each potentially representing a different proof system.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord, Serialize, Deserialize)]
pub struct ExecutionProofSubnetId(u8);

// Manual SSZ implementation for newtype wrapper
...
impl ExecutionProofSubnetId {
    /// Creates a new ExecutionProofSubnetId if the value is valid (0-7)
...
mod tests {
    ...
    fn test_invalid_subnet_ids() {
        ...
        assert!(ExecutionProofSubnetId::new(255).is_err());
    }
    ...
    fn test_conversions() {
        ...
        assert_eq!(subnet_id.as_u8(), 3);
        assert_eq!(subnet_id.as_usize(), 3);

        let as_u8: u8 = subnet_id.into();
        assert_eq!(as_u8, 3);

        let from_u8 = ExecutionProofSubnetId::try_from(3u8).unwrap();
        assert_eq!(from_u8, subnet_id);
    }
}
