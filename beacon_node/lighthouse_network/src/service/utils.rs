...
use types::{
    ChainSpec, DataColumnSubnetId, EnrForkId, EthSpec, ForkContext,
...
pub(crate) fn create_whitelist_filter(
    ...
    for fork_digest in possible_fork_digests {
        ...
        // TODO(zkproofs): Check stateless-validation flag before adding this
        // Add execution proof topic (single topic, not subnet-based)
        add(ExecutionProof);
    }
