...
///
/// Stores proofs indexed by execution block hash. Multiple proofs from
/// different subnets can exist for the same block hash.
...
impl ProofCache {
    ...
    ///
    /// If proofs for this block_hash already exist, the new proof is added
    /// to the vector (unless a proof from the same subnet already exists).
    pub async fn insert(&self, proof: ExecutionProof) {
        ...
        cache
            ...
            .retain(|p| p.subnet_id != proof.subnet_id); // Remove old proof from same subnet
    ...
    /// Check if we have the minimum required number of proofs from DIFFERENT subnets
    ...
    /// Get the number of unique subnets we have proofs from for a block
...
mod tests {
    ...
    async fn test_cache_replace_same_subnet() {
        ...
        let proof_1 = create_test_proof(subnet_0, block_hash);
        let proof_2 = create_test_proof(subnet_0, block_hash);

        cache.insert(proof_1).await;
        cache.insert(proof_2).await;
    ...
    async fn test_cache_lru_eviction() {
        let cache = ProofCache::new(2); // Small cache
        let subnet_0 = ExecutionProofSubnetId::new(0).unwrap();
        let block_hash_1 = ExecutionBlockHash::repeat_byte(1);
        let block_hash_2 = ExecutionBlockHash::repeat_byte(2);
        ...
        // block_hash_1 should be evicted (least recently used)
