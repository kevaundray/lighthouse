...
#[async_trait]
impl ProofGenerator for DummyGenerator {
    async fn generate(
        &self,
...
mod tests {
    ...
    async fn test_dummy_generator_success() {
        ...
        let generator = DummyGenerator::new(subnet);
    ...
    fn test_generator_properties() {
        let subnet = ExecutionProofSubnetId::new(2).unwrap();
        let generator = DummyGenerator::new(subnet);

        assert_eq!(generator.subnet_id(), subnet);
        assert_eq!(generator.name(), "DummyGenerator");
    }
    ...
    async fn test_dummy_generator_custom_delay() {
        ...
        let generator = DummyGenerator::with_delay(subnet, delay);
        let block_hash = ExecutionBlockHash::repeat_byte(1);
        let block_root = Hash256::repeat_byte(2);
